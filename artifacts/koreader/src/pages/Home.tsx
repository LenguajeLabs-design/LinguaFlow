...
import { BookOpen, Sparkles, TrendingUp, Library, Bookmark, PlusCircle, Star, BookMarked } from 'lucide-react';
...
export default function Home() {
  ...
  const heroGradient = isZh
    ? 'linear-gradient(135deg, hsl(0,70%,46%) 0%, hsl(15,80%,52%) 35%, hsl(30,90%,60%) 70%, hsl(45,85%,62%) 100%)'
    : 'linear-gradient(135deg, hsl(174,62%,42%) 0%, hsl(200,68%,50%) 35%, hsl(255,52%,58%) 70%, hsl(295,42%,58%) 100%)';

  const orb1Color = isZh ? 'hsl(0,70%,55%)' : 'hsl(174,60%,52%)';
  const orb2Color = isZh ? 'hsl(45,85%,65%)' : 'hsl(295,42%,67%)';
      ...
      <div className="space-y-16 pb-12">
        ...
        <section className="relative rounded-3xl overflow-hidden shadow-2xl">
          ...
          <div className="hf-orb absolute -top-20 -left-16 w-72 h-72 rounded-full"
            style={{ background: orb1Color, opacity: 0.25, filter: 'blur(60px)' }} />
          <div className="hf-orb-2 absolute -bottom-16 right-0 w-80 h-80 rounded-full"
            style={{ background: orb2Color, opacity: 0.20, filter: 'blur(72px)' }} />
          ...
          <div className="relative z-10 p-8 sm:p-12 md:p-16 max-w-2xl">
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-serif font-bold tracking-tight mb-6 leading-tight text-white drop-shadow-sm">
              {isZh
                ? <>Master Chinese<br className="hidden sm:block" /> through graded<br className="hidden sm:block" /> stories.</>
            ...
            <p className="text-lg sm:text-xl text-white/80 mb-8 max-w-xl font-light leading-relaxed">
              {isZh
                ? 'Generate Chinese passages calibrated to your HSK level on any topic. Build vocabulary naturally with clickable Hanzi and Pinyin.'
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Link href="/library" className="hf-btn-ghost">
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="hf-card hf-gradient-border flex items-center gap-4 p-6">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0"
              style={{ background: isZh ? 'linear-gradient(135deg,hsl(0,70%,50%),hsl(20,80%,58%))' : 'linear-gradient(135deg, hsl(174,60%,52%), hsl(200,68%,58%))' }}>
          ...
          <div className="hf-card hf-gradient-border flex items-center gap-4 p-6">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0"
              style={{ background: isZh ? 'linear-gradient(135deg,hsl(20,80%,55%),hsl(40,90%,60%))' : 'linear-gradient(135deg, hsl(255,52%,66%), hsl(295,42%,67%))' }}>
          ...
          <div className="hf-card hf-gradient-border flex items-center gap-4 p-6">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0"
              style={{ background: isZh ? 'linear-gradient(135deg,hsl(0,65%,50%),hsl(20,80%,55%))' : 'linear-gradient(135deg, hsl(200,68%,58%), hsl(255,52%,66%))' }}>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-3">
            {langConfig.topicSuggestions.map(topic => (
              <button
                key={topic}
            ...
            <div className="relative text-center py-20 rounded-3xl border border-dashed border-border overflow-hidden bg-card">
              <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full hf-orb"
                  style={{ background: isZh ? 'hsl(0,65%,52%)' : 'hsl(174,60%,52%)', opacity: 0.06, filter: 'blur(60px)' }} />
              </div>
              <div className="relative z-10">
                <div className="w-20 h-20 rounded-3xl mx-auto mb-6 flex items-center justify-center shadow-lg"
                  style={{ background: isZh ? 'linear-gradient(135deg,hsl(0,70%,46%),hsl(30,90%,58%))' : 'linear-gradient(135deg, hsl(174,62%,42%), hsl(255,52%,60%))' }}>
                ...
                <p className="text-muted-foreground max-w-sm mx-auto mb-8 leading-relaxed">
                  {isZh
                    ? 'Start your journey by generating your first Chinese reading passage, calibrated to your HSK level.'
                ...
                <Link
                  ...
                  style={{ background: isZh ? 'linear-gradient(135deg,hsl(0,70%,46%),hsl(15,80%,52%),hsl(30,90%,58%))' : 'linear-gradient(135deg, hsl(174,62%,42%), hsl(200,68%,52%), hsl(255,52%,60%))' }}
                >
                  <PlusCircle className="w-5 h-5" />
